...
async def dirbust_req(url, proxy, output, verbose, title, timeout, server, application_type, word_count, no_color, allow_redirect, match, exclude, session, sem, bar):
        ...
        async with sem:
            ...
            async with session.get(url, ssl=False, proxy=proxies, timeout=timeout, allow_redirects=redirect, max_redirects=10) as response:
                ...
                if not no_color:
                    ...
                    server = f" {bold}{white}-{reset} {bold}{white}[{reset}{bold}{white}{reset}{bold}{magenta}{server1}{reset}{bold}{white}]{reset} " if server else ""
                    
                else:
                    
                    server = f" - [{server1}] " if server else ""
                    
                if not no_color:
                    
                    content = f" {bold}{white}-{reset} {bold}{white}[{reset}{bold}{yellow}{content_type}{reset}{bold}{white}]{reset}" if application_type else ""
                    
                else:
                    
                    content = f" - [{content_type}]" if application_type else ""
                    
                if not no_color:
                    
                    word =  f" {bold}{white}-{reset} {bold}{white}[{reset}{bold}{green}{word_count}{reset}{bold}{white}]{reset}" if word_count else ""
                    
                else:
                    
                    word =  f" - [{word_count}]" if word_count else ""
                    
                if not no_color:
        
                    title = f" {bold}{white}-{reset} {bold}{white}[{reset}{bold}{cyan}{title}{reset}{bold}{white}]{reset}" if title else ""
